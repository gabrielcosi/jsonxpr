...
export const conditional: ConditionalType = {
  /**
  ...
  $cond(args: { if: Value; then: Value; else: Value }, vars: Variables): Value {
    const condition = jsonxpr(args.if, vars) as boolean;
    return condition ? jsonxpr(args.then, vars) : jsonxpr(args.else, vars);
  },
  ...
  $ifNull(args: Value[], vars: Variables): Value {
    for (const arg of args) {
      const value = jsonxpr(arg, vars);
      if (value !== null) return value;
    }
    return jsonxpr(args[args.length - 1], vars);
  },
  ...
  $switch(
    ...
    for (const branch of args.branches) {
      const condition = jsonxpr(branch.case, vars) as boolean;
      if (condition) return jsonxpr(branch.then, vars);
    }
    return jsonxpr(args.default, vars);
  },
};
