...
export const object: ObjectType = {
  ...
  $getField(args: { field: Value; input: Value }, vars: Variables): Value {
    const field = jsonxpr(args.field, vars) as string;
    const input = jsonxpr(args.input, vars) as Record<string, Value>;
  ...
  $mergeObjects(args: Value[], vars: Variables): Record<string, Value> {
    return args.reduce((acc: Record<string, Value>, arg) => {
      const object = jsonxpr(arg, vars) as Record<string, Value>;
      return { ...acc, ...object };
    }, {});
  },
  ...
  $setField(
    ...
    const field = jsonxpr(args.field, vars) as string;
    const input = jsonxpr(args.input, vars) as Record<string, Value>;
    const value = jsonxpr(args.value, vars);
