...
describe("operations: special", () => {
  it("$isValidEmail: Returns true if the string is a valid email address", () => {
    const validEmails = [
      `email@domain.com`,
    ...
    for (const email of validEmails) {
      expect(jsonxpr({ $isValidEmail: email })).toBe(true);
    }
  });

  it("$isValidEmail: Returns false if the string is not a valid email address", () => {
    const invalidEmails = [
    ...
    for (const email of invalidEmails) {
      expect(jsonxpr({ $isValidEmail: email })).toBe(false);
    }
  });

  it("$isValidPhone: Returns true if the string is a valid phone number", () => {
    const validPhones = [
      `123-456-7890`,
      `123.456.7890`,
      `123 456 7890`,
    ...
    for (const phone of validPhones) {
      expect(jsonxpr({ $isValidPhone: phone })).toBe(true);
    }
  });
});
